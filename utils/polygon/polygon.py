...
def correct_draw(path):
    ...
    new_image = cv2.rectangle(
        new_image, rest_segments[6][0], rest_segments[6][1], color, thickness)
    cv2.imwrite('aici.jpg', new_image)

    return True


print(correct_draw('./polygon.png'))

# new_image = rotate_image(image, rotations[3])
# new_image = cv2.rectangle(
#     new_image, segments[3][0], segments[3][1], color, thickness)
# cv2.imwrite('aici.jpg', new_image)
